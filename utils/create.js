...
function create(store, option) {
    if (arguments.length === 2) {
        ...
        option.onLoad = function (e) {
            ...
            const using = path_1.getUsing(store.data, option.use);
        ...
        store.ready = store.lifetimes.ready = function () {
            ...
            const using = path_1.getUsing(this.store.data, store.use);
...
function observeStore(store) {
    const oba = obaa_1.default(store.data, (prop, value, old, path) => {
        let patch = {};
...
function _updateOne(kv, store, ins) {
    if (store.updateAll || ins.__updatePath && path_1.needUpdate(kv, ins.__updatePath)) {
        ...
        const using = path_1.getUsing(store.data, ins.__use);
